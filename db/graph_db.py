...
Soporte para openCypher, almacenamiento binario Protobuf, ThreadedConnectionPool
y Fallback Resiliente en SQLite con WAL mode y Thread-Safety.
======================================================================
"""
...
class PostgresAGEManager:
    """
    Administrador Thread-Safe de la conexi?n con PostgreSQL y Apache AGE.
    Utiliza ThreadedConnectionPool para concurrencia de llamadas sin colisiones.
    """
    ...
    def _init_sqlite_fallback(self):
        """Inicializa almacenamiento local SQLite con WAL mode para concurrencia segura."""
    ...
    def _try_connect_postgres(self) -> bool:
        """Inicializa el ThreadedConnectionPool y configura la extensi?n Apache AGE."""
        try:
            import psycopg2
            ...
            try:
                ...
                self._use_fallback = False
                logger.info(f"? Conectado exitosamente a PostgreSQL + Apache AGE con ThreadedConnectionPool (Grafo: '{GRAPH_NAME}').")
                return True
            finally:
                self._pg_pool.putconn(conn)
        except Exception as e:
            logger.warning(f"?? No se pudo conectar a PostgreSQL + AGE ({e}). Activando motor de grafos en memoria y cach? local.")
    ...
    def is_age_connected(self) -> bool:
        return self._is_age_ready and not self._use_fallback and (self._pg_pool is not None)

    @contextmanager
    def _get_pg_connection(self):
        """Adquiere y libera de forma segura una conexi?n del pool."""
        if not self._pg_pool:
            raise RuntimeError("PostgreSQL Connection Pool no est? disponible.")
        conn = self._pg_pool.getconn()
        try:
            conn.autocommit = True
            yield conn
    ...
    @staticmethod
    def sanitize_cypher_literal(val: str) -> str:
        """Sanitiza cadenas literales para evitar inyecciones en consultas openCypher."""
        if not val:
            return ""
    ...
    def execute_cypher(self, query: str, return_cols: str = "v agtype") -> List[Any]:
        """
        Ejecuta una consulta openCypher thread-safe en Apache AGE.
        """
    ...
    def save_order(self, order_dict: Dict[str, Any], proto_bytes: bytes) -> bool:
        ...
        # Fallback local SQLite Thread-Safe
        try:
            with self._sqlite_lock:
                with self._sqlite_conn:
                    self._sqlite_conn.execute("""
    ...
    def save_call_log(self, call_data: Dict[str, Any], proto_bytes: Optional[bytes] = None) -> bool:
        """Persiste la telemetr?a, duraci?n y resumen de una llamada telef?nica."""
        ...
        if self.is_age_connected:
            try:
                ...
                with self._get_pg_connection() as conn:
                    ...
                    logger.info(f"?? [Call Log PG]: Sesi?n {session_id} | {turn_count} turnos | {duration:.1f}s")
                    return True
            except Exception as e:
    ...
    def update_customer_profile(self, phone: str, updates: Dict[str, Any]) -> bool:
        """Actualiza campos espec?ficos (direcci?n, notas, zona) del perfil de un cliente."""
    ...
    def save_learning_event(self, event_dict: Dict[str, Any], proto_bytes: bytes) -> bool:
        """Persiste un nuevo aprendizaje o correcci?n fon?tica sanitizada en la base de datos."""
        ...
        # 2. Inyectar nuevo nodo y arista en el Grafo de Apache AGE sanitizando entradas
        if self.is_age_connected:
            try:
                safe_alias = self.sanitize_cypher_literal(alias)
                safe_target = self.sanitize_cypher_literal(target)
                safe_etype = self.sanitize_cypher_literal(etype)
                cypher_alias = f"""
                    MERGE (a:Alias {{name: '{safe_alias}', entity_type: '{safe_etype}'}})
                    WITH a
                    MATCH (d:Dish) WHERE toLower(d.name) = toLower('{safe_target}')
                    MERGE (d)-[:HAS_ALIAS]->(a)
                    RETURN count(a)
                """
    ...
    def get_customer_profile(self, phone: str) -> Optional[Dict[str, Any]]:
        """Recupera el perfil, preferencias y direcci?n de un cliente recurrente."""
        if not phone or phone in ["Desconocido", "0000000000"]:
            return None
    ...
    def get_all_learned_aliases(self) -> Dict[str, str]:
        """Obtiene el mapa de alias fon?ticos acumulados."""
        aliases = {}
        with self._sqlite_lock:
            cur = self._sqlite_conn.cursor()
