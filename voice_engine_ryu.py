...
class RyuVoiceAgent:
    ...
    def think_and_respond(self, user_text: str) -> str:
        ...
        clean_user_text = re.sub(r"\barcogrid(?:\s*roll)?\b|\barco\s*grid(?:\s*roll)?\b", "Arcoíris Roll", clean_user_text, flags=re.IGNORECASE)
        clean_user_text = re.sub(r"\bcheese\s*roll\b|\brollo\s*cheese\b", "Cheese Explosion", clean_user_text, flags=re.IGNORECASE)
        clean_user_text = re.sub(r"\bqueso\s*con\s*chorizo\b", "queso fundido con chorizo", clean_user_text, flags=re.IGNORECASE)
        clean_user_text = re.sub(r"\btabla\s*de\s*30\s*(?:bocadillos|piezas)?\b|\btabla\s*30\s*(?:bocadillos|piezas)?\b|\b30\s*(?:bocadillos|piezas)\s*(?:de\s*sushi)?\b", "Tabla de Sushi Mixta", clean_user_text, flags=re.IGNORECASE)
        clean_user_text = re.sub(r"\bsalsa\s*siracha\b|\bsiracha\b", "salsa sriracha", clean_user_text, flags=re.IGNORECASE)
            ...
            extract_prompt = [
                {
                    ...
                    "content": (
                        ...
                        "• SUSHI Y ROLLOS: Empanizado $95, Philadelphia $95, California $95, Kani $95, Esfera Dragón Ball $95, Avocado $100, Chipotle $100, Arcoiris (Arcogrid) $100, Hulk $100, Eby $100, Plátano Roll $100, Salmón Roll $105, México Roll $105, Philadelphia Empanizado $105, Furia $105, Pink Salmón $105, Oishi $105, Flamin Hot $105, Masago $110, Empanizado Especial $115, Tocayo $115, Mar y Tierra $115, Mechudo $120, Eby Roll Especial $120, Especial Ryü $120, Especial No. 1 $120, Zizi Roll $130, Hulk Especial $130, Tuna Roll $130, Cheese Explosion (Cheese Roll) $135, Panchito $140, Tocino Roll $150, Tabla de Sushi Mixta (30 bocadillos: 1 Empanizado Especial, 1 Philadelphia, 1 Mechudo) $300.\n"
                        ...
                        "• MENÚ ITALIANO: Fettuccine Alfredo $100, Boloñesa $120, Lasagna $150, Panini (cualquier especialidad) $100, Pita (cualquier especialidad) $120 (ambos SÍ incluyen papas a la francesa; la diferencia de precio es exclusivamente por el pan: panini pan alargado crujiente vs pita suave; especialidades: Bbq, Chipotle, Cheesesteak, Pollo Clásico, Pollo Crispy, Carnes Frías), Ensaladas Italianas Bbq/Chipotle/Cheese-Ranch/Piña Hot/Crispy $110, Toppings Champiñones/Elotes/Cebollita $15 c/u, Sodas Italianas con Boba Fresa/Piña/Mora Azul $45 c/u.\n"
