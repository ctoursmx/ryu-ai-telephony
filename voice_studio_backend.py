...
def transcode_browser_audio(raw_audio_bytes: bytes) -> Tuple[bytes, bytes, float]:
    """
    Decodifica el audio enviado por el navegador web (WebM, Opus, MP4, WAV, OGG)
    usando PyAV / FFmpeg interno y genera:
...
def save_item_recording(item_id: str, raw_audio: bytes) -> dict:
    """Procesa, guarda y sincroniza la grabación de un ítem."""
    item_id = str(item_id).strip()
    if not item_id:
        raise ValueError("ID de ítem no válido.")

    alaw_bytes, wav_bytes, duration_sec = transcode_browser_audio(raw_audio)
